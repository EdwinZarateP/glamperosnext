...
interface Props {
  filtros?: string[];
  // ✅ En Next 15 puede llegar como objeto o como Promise (según desde dónde lo llames)
...
const CITY_ALIASES: Record<string, { lat: number; lng: number; nombre: string }> = {
  ...
  cali: { lat: 3.4516, lng: -76.532, nombre: "Cali" },
};
...
function construirQuery(
  ...
  if (huespedes && Number(huespedes) > 1) params.set("totalHuespedes", huespedes);
  if (aceptaMascotasQS === "true") params.set("aceptaMascotas", "true");
...
export default async function TarjetasEcommerceServer({ filtros = [], searchParams }: Props) {
  // ✅ Resolver searchParams SI llega como Promise (o usar {} si viene vacío)
