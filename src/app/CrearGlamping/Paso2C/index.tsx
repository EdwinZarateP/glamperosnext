...
const Paso2C: React.FC = () => {
  ...
  const manejarSubida = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imagenesCargadas.length + imagenesArray.length > 30) {
      Swal.fire("Error", `Tienes cupo para 30 imágenes en total.`, "error");
      return;
    }
  ...
  const calcularCupoRestante = () => 30 - imagenesCargadas.length;
